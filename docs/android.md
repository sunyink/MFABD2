...
只上传 Actions 产物。`publish=true` 的独立上传入口已禁用，会在构建前报错；
需要公开版本时使用 `install`，由主流程统一校验并发布桌面与安卓附件。
...
核对提交、版本、文件名及摘要后，与桌面 ZIP 一起上传到草稿，全部附件核验通过后才公开。
安卓失败或附件不匹配时停止发布；取消清理和重跑方式见 [发布保护与恢复](release-publication.md)。
